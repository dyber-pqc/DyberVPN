...
use crate::audit::{AuditConfig, AuditLogger, EventOutcome, EventType};
...
use boringtun::noise::{HybridHandshakeState, MlDsaKeyPair, Packet, Tunn, TunnResult as WgResult};
...
impl Daemon {
    ...
    pub fn new(config: TunnelConfig) -> TunnelResult<Self> {
        ...
        let is_server = config.listen_addr.port() != 0 && !config.peers.is_empty();
    ...
    pub fn set_policy(&mut self, config: PolicyConfig) {
        ...
        if config.enabled {
            tracing::info!(
                "Access control policy engine enabled (default: {})",
    ...
    pub fn init(&mut self) -> TunnelResult<()> {
        ...
        tracing::info!(
            ...
            fips_report.results.len(),
            fips_report.duration
        );
        self.audit.log_admin_action(
            EventType::DaemonStarted,
            &format!(
                "FIPS 140-3 POST passed: {} tests in {:.2?}",
                fips_report.results.len(),
                fips_report.duration
            ),
        );
        ...
        if self.connector_mode {
            ...
            if self.config.peers.is_empty() {
                ...
                broker_peer.allowed_ips = vec![(IpAddr::V4(Ipv4Addr::UNSPECIFIED), 0)];
            ...
            tracing::info!(
                "Connector UDP socket bound to ephemeral port {}",
                local_port
            );
            ...
            match ConnectorAgent::new(&connector_cfg) {
                ...
                Err(e) => {
                    ...
                    return Err(TunnelError::Other(format!(
                        "Broker control plane connection failed: {}",
                        e
                    )));
                }
            }
            ...
            self.audit.log_admin_action(
                EventType::DaemonStarted,
                &format!(
                    "Connector mode started (mode={}, broker={})",
    ...
    fn init_peers(&mut self) -> TunnelResult<()> {
        let our_mldsa_keypair = if let Some(ref key_bytes) = self.config.mldsa_private_key {
            match MlDsaKeyPair::from_secret_key_bytes(key_bytes) {
                Ok(kp) => {
                    tracing::info!("Loaded ML-DSA signing keypair");
                    Some(kp)
                }
                Err(e) => {
                    tracing::warn!("Failed to load ML-DSA keypair: {}", e);
                    None
                }
            }
        } else {
        ...
        for (idx, peer_config) in self.config.peers.iter().enumerate() {
            ...
            let peer_mldsa_public = if let Some(ref key_bytes) = peer_config.mldsa_public_key {
                match MlDsaPublicKey::from_bytes(key_bytes) {
                    Ok(pk) => Some(pk),
                    Err(e) => {
                        tracing::warn!("Failed to load peer ML-DSA key: {}", e);
                        None
                    }
                }
            } else {
    ...
    fn run_poll_loop(&mut self) -> TunnelResult<()> {
        ...
        tracing::info!(
            ...
            if self.audit.is_enabled() { "on" } else { "off" },
            if self.revocation.auto_disconnect() {
                "on"
            } else {
                "off"
            },
    ...
    fn run_busy_loop(&mut self) -> TunnelResult<()> {
        ...
        while !self.shutdown.load(Ordering::Relaxed) {
            ...
            if let Ok((n, src)) = self.socket.as_ref().unwrap().recv_from(&mut udp_buf) {
    ...
    fn handle_reload(&mut self, out_buf: &mut [u8]) {
        let config_path = match &self.config_path {
            Some(p) => p.clone(),
            None => {
                tracing::warn!("Reload requested but no config path set");
                return;
            }
        };
        ...
        // Reload peers
        let mut new_peer_keys: HashMap<[u8; 32], &dybervpn_protocol::config::PeerConfig> =
            HashMap::new();
        ...
        // Audit: config reload
        self.audit
            .log_config_reload(added, removed, self.peers.len());
    }
    ...
    fn add_peer_from_proto_config(
        ...
        if pk_bytes.len() != 32 {
            return Err(TunnelError::Config("Bad key len".into()));
        }
        ...
        let endpoint = if let Some(ref ep) = peer_cfg.endpoint {
            Some(
                ep.parse::<SocketAddr>()
                    .map_err(|e| TunnelError::Config(format!("Bad endpoint: {}", e)))?,
            )
        } else {
            None
        };

        let allowed_ips: Vec<(IpAddr, u8)> = peer_cfg
            .allowed_ips
            .split(',')
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
    ...
    fn initiate_handshakes(&mut self, out_buf: &mut [u8]) {
        let peer_endpoints: Vec<([u8; 32], SocketAddr)> = self
            .peers
            .iter()
            ...
            if self.policy.is_enabled() {
                if let Some((_src, dst, dst_port, proto)) = policy::inspect_packet(packet) {
                    ...
                    let (action, rule_name) =
                        self.policy
                            .evaluate(&key, peer_name, dst, dst_port, Some(proto));
    ...
    fn handle_udp_packet(&mut self, packet: &[u8], src: SocketAddr, out_buf: &mut [u8]) {
        ...
        if let Some(key) = peer_key {
            ...
            if self.revocation.is_revoked(&key) {
                let peer_name = self.peers.get(&key).and_then(|p| p.name.as_deref());
                tracing::warn!(
                    "Dropping packet from revoked peer {}",
                    hex::encode(&key[..4])
                );
                self.audit.log_handshake(
                    &key,
            ...
            let (result_action, is_handshake_complete) = {
                if let Some(peer) = self.peers.get_mut(&key) {
                    ...
                    if peer.endpoint != Some(src) {
                        self.audit.log_endpoint_changed(
                            &key,
                            peer.name.as_deref(),
                            peer.endpoint,
                            src,
                    ...
                    match result {
                        ...
                        | WgResult::WriteToTunnelV6(decrypted, _) => {
                            if !peer.session_established {
                                ...
                                self.audit
                                    .log_session_established(&key, peer.name.as_deref(), src);
                            }
            ...
            match result_action {
                ResultAction::TunnelPacket(decrypted) => {
                    ...
                    if self.policy.is_enabled() {
                        if let Some((_src_ip, dst_ip, dst_port, proto)) =
                            policy::inspect_packet(&decrypted)
                        {
                            let peer_name = self.peers.get(&key).and_then(|p| p.name.as_deref());
                    ...
                    if should_forward && !self.try_forward_to_peer(&decrypted, &key, out_buf) {
    ...
    fn try_forward_to_peer(
        ...
        if self.policy.is_enabled() {
            if let Some((_src_ip, dst, dst_port, proto)) = policy::inspect_packet(decrypted_packet)
            {
                ...
                let (action, rule_name) =
                    self.policy
                        .evaluate(source_peer_key, peer_name, dst, dst_port, Some(proto));
                if action == PolicyAction::Deny {
                    self.audit.log_policy_decision(
                        source_peer_key,
                        peer_name,
    ...
    fn handle_timers(&mut self, out_buf: &mut [u8]) {
        ...
        if now.duration_since(self.last_crngt_check) >= CRNGT_INTERVAL {
            ...
            if let Err(e) = dybervpn_protocol::fips::crngt_runtime_check(backend.as_ref()) {
                tracing::error!(
                    "FIPS 140-3 CRNGT runtime failure: {}. Initiating shutdown.",
                    e
                );
                self.audit.log_admin_action(
        ...
        let peer_data: Vec<([u8; 32], Option<SocketAddr>, Option<Instant>, bool)> = self
            .peers
            .iter()
            .map(|(pk, peer)| {
                (
            ...
            if session_ok {
                if let Some(hs_time) = last_hs {
                    ...
                    if age > session_max_age {
                        ...
                        tracing::info!(
                            "Peer {} session expired after {} hours",
                            hex::encode(&pk[..4]),
                            age_hours
                        );
                        self.audit.log_session_expired(
                            &pk,
    ...
    fn scan_revoked_peers(&mut self) {
        ...
        for pk in &peer_keys {
            let status = self
                .revocation
                .check_key(pk, self.peers.get(pk).and_then(|p| p.name.as_deref()));
            match status {
                KeyStatus::Revoked(reason) => {
                    tracing::warn!(
                        "Disconnecting peer {} — key revoked: {}",
                        hex::encode(&pk[..4]),
                        reason
                    );
                    self.audit.log_key_revoked(
                        pk,
                        self.peers.get(pk).and_then(|p| p.name.as_deref()),
                        &reason,
                    );
                ...
                KeyStatus::Expired(detail) => {
                    tracing::warn!(
                        "Disconnecting peer {} — key expired: {}",
                        hex::encode(&pk[..4]),
                        detail
                    );
                    self.audit.log_key_revoked(
        ...
        if !revoked_keys.is_empty() {
            tracing::info!(
                "Revocation scan: removed {} peers, {} remaining",
                revoked_keys.len(),
                self.peers.len()
            );
        }
    ...
    fn get_dst_ip(packet: &[u8]) -> TunnelResult<IpAddr> {
        if packet.is_empty() {
            return Err(TunnelError::InvalidPacket("Empty".into()));
        }
        let version = packet[0] >> 4;
        match version {
            4 if packet.len() >= 20 => Ok(IpAddr::V4(Ipv4Addr::new(
...
mod tests {
    ...
    fn test_get_dst_ip() {
        ...
        assert_eq!(
            Daemon::get_dst_ip(&packet).unwrap(),
            IpAddr::V4(Ipv4Addr::new(192, 168, 1, 1))
        );
    }
    ...
    fn test_two_daemons_classic_handshake_and_data() {
        ...
        assert!(
            ...
            "Client should have transmitted bytes"
        );
        ...
        let result = server_peer
            .tunn
            .decapsulate(Some(src.ip()), &recv_buf[..n], &mut decrypt_buf);
    ...
    fn test_two_daemons_bidirectional_traffic() {
        ...
        match result {
            ...
            other => panic!(
                "c→s: expected WriteToTunnelV4, got {:?}",
                std::mem::discriminant(&other)
            ),
        }
        ...
        match result {
            ...
            other => panic!(
                "s→c: expected WriteToTunnelV4, got {:?}",
                std::mem::discriminant(&other)
            ),
        }
    }
