...
impl MacOsTun {
    ...
    pub fn create(name: &str) -> TunnelResult<Self> {
        ...
        let ret = unsafe {
            libc::ioctl(fd, CTLIOCGINFO, &mut info as *mut CtlInfo)
        };
...
impl TunDevice for MacOsTun {
    ...
    fn set_address(&self, addr: IpAddr, prefix: u8) -> TunnelResult<()> {
        match addr {
            IpAddr::V4(v4) => {
                ...
                self.run_ifconfig(&["inet", &addr_str, &addr_str, "netmask", &prefix_to_mask_v4(prefix)])
            }
            IpAddr::V6(v6) => {
    ...
    fn read(&self, buf: &mut [u8]) -> TunnelResult<usize> {
        ...
        let n = unsafe {
            libc::read(self.fd, full_buf.as_mut_ptr() as *mut libc::c_void, full_buf.len())
        };
    ...
    fn write(&self, buf: &[u8]) -> TunnelResult<usize> {
        ...
        let n = unsafe {
            libc::write(self.fd, full_buf.as_ptr() as *const libc::c_void, full_buf.len())
        };
