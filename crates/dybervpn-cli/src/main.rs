...
use dybervpn_tunnel::{Daemon, PeerConfig, TunnelConfig};
use std::fs;
use std::io::Write;
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Init {
            ...
            client,
            output,
        } => cmd_init(server, client.as_deref(), &output),
        ...
        Commands::AddPeer {
            ...
            endpoint,
            output,
        } => cmd_add_peer(&config, &name, &endpoint, &output),
        ...
        Commands::Enroll {
            ...
            mode,
            output,
        } => cmd_enroll(&server, &token, &name, &mode, &output),
        Commands::RevokeKey {
            config,
            peer,
            reason,
            revoked_by,
            yes,
        } => cmd_revoke_key(&config, &peer, &reason, revoked_by.as_deref(), yes),
        Commands::SuspendKey {
            config,
            peer,
            expires,
            revoked_by,
        } => cmd_suspend_key(&config, &peer, expires.as_deref(), revoked_by.as_deref()),
...
fn cmd_genkey(mode: &str, format: &str) -> Result<()> {
    ...
    if mode.uses_pq_kex() && format == "toml" {
        ...
        println!(
            "# pq_public_key = \"{}\"",
            base64::encode(mlkem_pub.as_bytes())
        );
    }
    ...
    if mode.uses_pq_auth() && format == "toml" {
        ...
        println!(
            "mldsa_private_key = \"{}\"",
            base64::encode(mldsa_priv.as_bytes())
        );
        println!(
            "# mldsa_public_key = \"{}\"",
            base64::encode(mldsa_pub.as_bytes())
        );
    }
...
fn cmd_add_peer(
    ...
    let server_config_str = fs::read_to_string(server_config_path).with_context(|| {
        format!(
    ...
    let server_config: Config =
        toml::from_str(&server_config_str).context("Failed to parse server config")?;
    ...
    let server_pq_public_b64 = if let Some(ref pq_priv) = server_config.interface.pq_private_key {
        ...
        let pq_priv_bytes =
            base64::decode(pq_priv).context("Invalid server pq_private_key base64")?;
        ...
        if pq_priv_bytes.len() >= 2400 {
            ...
            anyhow::bail!(
                "Cannot derive PQ public key from server config. \
    ...
    let client_ip = format!(
        "{}.{}.{}.{}",
        base_octets[0], base_octets[1], base_octets[2], next_octet
    );
    let client_cidr = format!("{}/{}", client_ip, prefix);
    let subnet = format!(
        "{}.{}.{}.0/{}",
        base_octets[0], base_octets[1], base_octets[2], prefix
    );

    // Generate client keys
    let (client_x25519_pub, client_x25519_priv) = backend
        .x25519_keygen()
        .context("Failed to generate X25519 key")?;
    let (_client_ed25519_pub, _client_ed25519_priv) = backend
        .ed25519_keygen()
        .context("Failed to generate Ed25519 key")?;
    ...
    let mut server_file = fs::OpenOptions::new()
        ...
        .with_context(|| {
            format!(
                "Failed to open server config for writing: {}",
                server_config_path.display()
            )
        })?;

    server_file
        .write_all(peer_block.as_bytes())
    ...
    fs::write(&client_config_path, &client_config).with_context(|| {
        format!(
    ...
    // Summary
    println!(
        "\x1b[1;32m✓ Peer '{}' added successfully\x1b[0m",
        client_name
    );
    println!();
    ...
    println!("Next steps:");
    println!(
        "  1. Copy {} to the client machine",
        client_config_path.display()
    );
    println!(
        "  2. On the client: sudo dybervpn up -c {} -f",
        client_config_path.file_name().unwrap().to_string_lossy()
    );
    println!("  3. Apply changes to a running server (no restart needed):");
    println!("     dybervpn reload dvpn0");
    println!(
        "     — or restart: sudo dybervpn down dvpn0 && sudo dybervpn up -c {} -f",
        server_config_path.display()
    );
...
fn cmd_up(config_path: &PathBuf, foreground: bool) -> Result<()> {
    ...
    if !foreground {
        ...
        println!(
            "Starting DyberVPN tunnel '{}' in background...",
            interface_name
        );
        ...
        tracing::info!(
            "Starting DyberVPN tunnel '{}' in foreground",
            interface_name
        );
    }
    ...
    let mut daemon = Daemon::new(tunnel_config).context("Failed to create daemon")?;
    ...
    // ── Enterprise: Revocation Engine ──────────────────────────────────
    {
        ...
        if config.security.crl_path.is_some() {
            tracing::info!(
                "Key revocation engine enabled (CRL: {})",
    ...
    if config.audit.enabled {
        ...
        let categories: Vec<EventCategory> = config
            .audit
            .events
            .iter()
            .filter_map(|s| {
                match s.to_lowercase().as_str() {
                    ...
                    _ => {
                        tracing::warn!("Unknown audit event category: {}", s);
                        None
                    }
                }
            })
    ...
    #[cfg(unix)]
    {
        ...
        RELOAD_FLAG.store(
            Box::into_raw(Box::new(reload_flag_clone)) as usize,
            Ordering::SeqCst,
        );
    }
    ...
    if config.enrollment.enabled {
        let enrollment_token = config.enrollment.token.clone().unwrap_or_else(|| {
        ...
        let enrollment_listen: SocketAddr = config
            .enrollment
            .listen
            .parse()
        ...
        let server_endpoint = config
            ...
            .clone()
            .unwrap_or_else(|| {
                format!("0.0.0.0:{}", config.interface.listen_port.unwrap_or(51820))
            });
...
fn convert_config(config: &Config) -> Result<TunnelConfig> {
    ...
    let private_key_bytes =
        base64::decode(&config.interface.private_key).context("Invalid private_key base64")?;
    ...
    let connector = if let Some(ref conn) = config.connector {
        ...
        let broker_endpoint: SocketAddr = conn
            .broker_endpoint
            .parse()
            .context("Invalid connector.broker_endpoint")?;
        let broker_control: SocketAddr = conn
            .broker_control
            .parse()
    ...
    for peer_config in &config.peer {
        let public_key_bytes =
            base64::decode(&peer_config.public_key).context("Invalid peer public_key base64")?;
        ...
        let allowed_ips: Vec<(IpAddr, u8)> = peer_config
            .allowed_ips
            .split(',')
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
...
fn cmd_down(interface: &str) -> Result<()> {
    ...
    let pid_file_to_use = if pid_path.exists() {
        ...
        if primary.exists() {
            ...
            eprintln!();
            eprintln!(
                "To stop a foreground tunnel, use Ctrl+C in the terminal where it's running."
            );
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                if let Ok(output) = output {
                    if output.status.success() {
                        eprintln!();
                        eprintln!(
                            "Note: Interface '{}' exists. You may need to remove it manually:",
                            interface
                        );
...
fn cmd_init(server: bool, client: Option<&str>, output: &PathBuf) -> Result<()> {
    ...
    let config = if server {
        format!(
            r#"# DyberVPN Server Configuration
# Generated: {}
...
"#,
        ...
        format!(
            r#"# DyberVPN Client Configuration
# Generated: {}
...
"#,
    ...
    println!(
        "  ML-KEM:    {}...",
        &base64::encode(mlkem_pub.as_bytes())[..40]
    );
    println!();
    ...
    if server {
        ...
        println!(
            "  2. Start the server: dybervpn up -c {}",
            config_path.display()
        );
    } else {
        println!("Next steps:");
...
fn cmd_remove_peer(config_path: &Path, peer_identifier: &str, skip_confirm: bool) -> Result<()> {
    ...
    let matched: Vec<_> = peer_blocks
        .iter()
        .enumerate()
        .filter(|(_, (_, _, name, pub_key, allowed_ips))| {
...
fn cmd_reload(interface: &str) -> Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        if !is_process_running(pid) {
            anyhow::bail!(
                "Process {} is not running. The tunnel may have stopped.",
                pid
            );
        }
        ...
        println!(
            "\x1b[1;32m✓ Sent reload signal to '{}' (PID {})\x1b[0m",
            interface, pid
        );
        println!();
...
fn cmd_enroll(server_url: &str, token: &str, name: &str, mode: &str, output: &Path) -> Result<()> {
    ...
    let (x25519_pub, x25519_priv) = backend.x25519_keygen().context("X25519 keygen failed")?;
    ...
    let client_config_template = enroll_response["client_config"]
        .as_str()
...
fn cmd_sign_keygen(output: &Path, name: &str) -> Result<()> {
    ...
    println!(
        "  Verify: dybervpn verify <file> --key {}",
        pk_path.display()
    );
...
fn cmd_sign(file: &Path, key_path: &Path, output: Option<&Path>) -> Result<()> {
    ...
    let sk = MlDsaSecretKey::from_bytes(&sk_bytes).context("Invalid ML-DSA-65 private key")?;
    ...
    // Sign with ML-DSA-65
    eprintln!(
        "Signing {} ({} bytes) with ML-DSA-65...",
        file.display(),
        file_size
    );

    let signature = backend
        .mldsa_sign(&sk, &sign_message)
    ...
    println!("Verify with:");
    println!(
        "  dybervpn verify {} --key <public_key>.mldsa.pub",
        file.display()
    );
...
fn cmd_verify(file: &Path, key_path: &Path, sig_path: Option<&Path>) -> Result<()> {
    ...
    let pk = MlDsaPublicKey::from_bytes(&pk_bytes).context("Invalid ML-DSA-65 public key")?;
    ...
    let sig_data = fs::read(&sig_file_path).with_context(|| {
        format!(
            "Failed to read signature file: {}\n\
            Looked for: {}\n\
            Specify manually with --sig <path>",
    ...
    let signature =
        MlDsaSignature::from_bytes(sig_bytes).context("Invalid ML-DSA-65 signature in file")?;
    ...
    let valid = backend
        .mldsa_verify(&pk, sign_message, &signature)
...
fn resolve_peer_from_config(config_path: &Path, identifier: &str) -> Result<([u8; 32], String)> {
    ...
    for (i, peer) in config.peer.iter().enumerate() {
        ...
        let name = peer.name.as_deref().or_else(|| {
            peer_names
                .get(i)
                .and_then(|n| if n.is_empty() { None } else { Some(n.as_str()) })
        });
        ...
        let matches_name = name
            ...
            .unwrap_or(false);
        let matches_key =
            peer.public_key.to_lowercase().starts_with(&id_lower) || pk_hex.starts_with(&id_lower);
    ...
    match matches.len() {
        ...
        n => {
            eprintln!(
                "Multiple peers ({}) match '{}'. Be more specific.",
                n, identifier
            );
...
fn cmd_revoke_key(
    ...
    engine
        .revoke_key(&peer_key, Some(&peer_name), reason_enum, revoked_by)
...
fn cmd_suspend_key(
    ...
    let expires_rfc3339 = expires.map(|e| {
        ...
        let duration = if let Some(h) = e_lower.strip_suffix('h') {
            h.parse::<i64>()
                .ok()
                .and_then(|n| chrono::TimeDelta::try_hours(n))
        } else if let Some(d) = e_lower.strip_suffix('d') {
            d.parse::<i64>()
                .ok()
                .and_then(|n| chrono::TimeDelta::try_days(n))
        } else if let Some(w) = e_lower.strip_suffix('w') {
            w.parse::<i64>()
                .ok()
                .and_then(|n| chrono::TimeDelta::try_weeks(n))
        } else {
            None
        };
        match duration {
            ...
            None => {
                eprintln!(
                    "Warning: could not parse expiry '{}', suspension will be indefinite",
                    e
                );
                String::new()
            }
    ...
    engine
        .suspend_key(&peer_key, Some(&peer_name), expires_str, revoked_by)
...
fn cmd_reinstate_key(config_path: &Path, peer_identifier: &str) -> Result<()> {
    ...
    if !engine.is_revoked(&peer_key) {
        println!(
            "Peer '{}' ({}) is not currently revoked or suspended.",
            peer_name,
...
fn cmd_self_test(json: bool) -> Result<()> {
    ...
    if json {
        ...
        let json_results: Vec<serde_json::Value> = report
            .results
            .iter()
            .map(|r| {
                serde_json::json!({
...
fn cmd_connect(config_path: &PathBuf, mode_override: Option<&str>) -> Result<()> {
    ...
    let mut daemon = Daemon::new(tunnel_config).context("Failed to create connector daemon")?;
...
fn cmd_broker(config_path: &PathBuf) -> Result<()> {
    ...
    tracing::info!(
        "Starting ZTNA Broker with config: {}",
        config_path.display()
    );

    let config_str =
        fs::read_to_string(config_path).context("Failed to read broker config file")?;
    let config_file: BrokerConfigFile =
        toml::from_str(&config_str).context("Failed to parse broker config TOML")?;

    let section = &config_file.broker;

    // Decode private key
    let pk_bytes =
        base64::decode(&section.private_key).context("Invalid base64 in broker private_key")?;
    if pk_bytes.len() != 32 {
    ...
    let listen_udp: SocketAddr = section
        .listen_udp
        .parse()
        .context("Invalid listen_udp address")?;
    let listen_control: SocketAddr = section
        .listen_control
        .parse()
    ...
    let broker_config = BrokerConfig {
        ...
        listen_control,
        private_key,
        pq_private_key: section
            .pq_private_key
            .as_ref()
            .and_then(|s| base64::decode(s).ok()),
        mldsa_private_key: section
            .mldsa_private_key
            .as_ref()
            .and_then(|s| base64::decode(s).ok()),
        mode: section.mode,
